...
// Function to get formatted year and week (YYYY-Week)
String getFormattedYearWeek(int week) {
  time_t epochTime = timeClient.getEpochTime(); // Get the epoch time from NTP
  struct tm *timeinfo = localtime(&epochTime);  // Convert to local time

  char buffer[16];
  strftime(buffer, 16, "%Y-W", timeinfo); // %Y for year and custom W prefix
  String yearWeek = String(buffer) + (week < 10 ? "0" : "") + String(week);
  Serial.println("Year Week: " + yearWeek);
  return yearWeek;
}
